---
title: Minhash Function & Coprime Numbers
...
$$
\begin{align*}
(3, 4) &\implies f(x) = (3x + 4) \% 12 \\
x = 0, 1, 2 ... 11 &\implies f(x) = 4, 7, 10, 1, 4, 7, 10, 1, 4, 7, 10, 1
\end{align*}
$$
...
$$
\begin{align*}
(7, 4) &\implies f(x) = (7x + 4) \% 12 \\
x = 0, 1, 2 ... 11 &\implies f(x) = 4, 11, 6, 1, 8, 3, 10, 5, 0, 7, 2, 9
\end{align*}
$$
...
$$
\begin{align*}
(7, 5) &\implies f(x) = (7x + 5) \% 12 \\
x = 0, 1, 2 ... 11 &\implies f(x) = 5, 0, 7, 2, 9, 4, 11, 6, 1, 8, 3, 10
\end{align*}
$$
...
Finally, we need to prove what we found.

## Hypothesis:

Essentially, this problem can be rewritten as the following hypothesis:

>
####Given a function $$f(x) = (ax + b) \% n$$, where $$n \in \mathbb{N}^*$$, and $$a,b \in \mathbb{N}$$
>
####$$a$$ and $$n$$ are co-prime $$\iff \not\exists x_1, x_2 \in \{0, 1, ..., n-1\}$$, $$x_1 \neq x_2$$, such that $$f(x_1) = f(x_2)$$ (no collision, perfect hashing)
...
###Sufficiency:
>
####$$\exists x_1, x_2 \in [0, n-1], x_1 \neq x_2$$, such that $$f(x_1) = f(x_2) \implies a$$ and $$n$$ are *NOT* coprime

Proof:

$$
\begin{align*}
f(x_1) = f(x_2) &\implies (a\cdot x_1) \% n = (a\cdot x_2) \% n \\
&\implies a\cdot x_1 - k\cdot n = a\cdot x_2 - k^\prime \cdot n \\
&\implies a\cdot (x_1 - x_2) = (k - k^\prime) \cdot n
\end{align*}
$$
...
Knowing that $$x_1, x_2 \in \{0, 1, ..., n - 1\}$$, then $$x_1 - x_2 \le n-1 \implies$$ *contradiction*
...
###Necessity:
>
####$$a$$ and $$n$$ are *NOT* co-prime $$\implies \exists x_1, x_2 \in {0,1,2, ..., n-1}, x_1 \neq x_2$$, such that $$f(x_1) = f(x_2)$$
...
then, $$ (ax + b)\% n = y \implies \exists k \in \mathbb{Z}$$, such that $$ a \cdot x + b = n \cdot k + y$$, where $$y \in \{0,1,2, ... n-1\}$$

can be rewritten as:

$$p \cdot q \cdot x + b = p \cdot q^\prime \cdot k + y \iff p \cdot (q \cdot x-q^\prime \cdot k) = y - b$$
...
$$
\begin{align*}
y = (a \cdot x + b) \% n = [(a \cdot x \% n) + (b \% n)] \% n &\implies (a \cdot x \% n) + (b \% n) = k \cdot n + y \\ &\implies (a \cdot x - k^\prime \cdot n) + \delta_{b,n} = k \cdot n + y \\
&\implies a \cdot x + \delta_{b,n} = (k + k^\prime) \cdot n + y
\end{align*}
$$

, where $$\delta_{b,n} = b \% n, \quad k \in \mathbb{Z}, \quad k^\prime \in \mathbb{Z}$$

now for a given $$y$$, we can take a $$b$$ such that, $$\delta_{b,n} = y$$, that leads to
...
1. $$ f(0) = 0, f(1) = 2, f(2) = 1, f(3) = 3 $$

2. $$ f(0) = 0, f(1) = 3, f(2) = 1, f(3) = 2 $$
...
$$f(0) = 0 \implies a\cdot 0 = 4k + 0 \implies k = 0$$

But,

$$f(1) = 2 \implies a\cdot 1 = 4k + 2$$

has no solution, since $$a$$ must be a odd number (a prime number which is not $$2$$) and $$4k + 2 = 2 (2k +1) $$ must be a even number.
...
Let's take again the example $$n=4$$, $$\{1, 3\}$$ is validate step. So $$n \cdot p == 4 \cdot 2 = 8$$. As a result, for $$n=4$$, there are $$ 8/4! = 8/24 = 1/3$$ distinct sequences can be reached by our hash function model.
